...
function Blog({blog}){
    ...
    const sendLike = async()=>{
        await axios.post('https://yetublog.com/like/',data)
        .then((response)=> alert('Done, You liked this blog'))
        .catch((error)=>alert(error))
    }
    const sendDislike = async()=>{
        await axios.post('https://yetublog.com/dislike/',data)
        .then((response)=> alert('Done, You disliked this blog'))
        .catch((error)=>alert(error))
    }
    // rendered component
    return(
        <View style={{ flexDirection:'row' }} key={blog.id}>
            <View style={{
                flexDirection: 'column',alignItems:'center',
                marginHorizontal: 5,
                marginTop:100,
                paddingHorizontal: 2, width: windowWidth>400?100: 100,
                ...
                height: 200,
                    ...
                    <View style={{ alignItems: 'center',marginHorizontal: 10, marginVertical: 10}}>
                            <TouchableOpacity onPress={sendLike}>
                                <Feather name="thumbs-up" size={24} color={'#121212'} />
                            </TouchableOpacity>
                            <Text style={blogContentStyle.dateText}>Like</Text>
                    </View>
                    <View style={{ alignItems: 'center',marginHorizontal: 10, marginVertical: 10 }}>
                            <TouchableOpacity onPress={sendDislike}>
                                <Feather name="thumbs-down" size={24} color={'#121212'} />
                            </TouchableOpacity>
                            <Text style={blogContentStyle.dateText}>Dislike</Text>
                    </View>
                    <View style={{ alignItems: 'center',marginHorizontal: 10, marginVertical: 10 }}>
                            <TouchableOpacity>
                                <Feather name="message-circle" size={24} color={'#121212'} />
                            </TouchableOpacity>
                            <Text style={blogContentStyle.dateText}>comment</Text>
                    </View>
            </View>
            <View style={blogContentStyle.container}>
                {/* Upper Wrapper */}
                <View style={blogContentStyle.upperWrapper}>
                    <View style={blogContentStyle.bioWrapper}>
                        <View style={{ width: windowWidth>800?300:300 }}>
                ...
                <View style={blogContentStyle.lowerWrapper}>
                    <View style={{ width:windowWidth>800? 700:windowWidth>400?400: 300, }}>
